...
public class SideMenu {
    ...
    private HBox userImg;
    private HBox userLabel;
    private HBox newTicketImg;
    private HBox newTicketLabel;


    public SideMenu(ClientViewModel clientViewModel){
    ...
    private VBox createMenu(){
        ...
        userLabel = imgLabel(ClientProperties.getInstance().getClient().getUsername());

        GridPane client = new GridPane();
        client.addColumn(2, userImg, userLabel);
        client.setPadding(new Insets(5));

        newTicketImg = imgBox("M140.744,0C63.138,0,0,63.138,0,140.744s63.138,140.744,140.744,140.744s140.744-63.138,140.744-140.744 S218.351,0,140.744,0z M140.744,263.488C73.063,263.488,18,208.426,18,140.744S73.063,18,140.744,18 s122.744,55.063,122.744,122.744S208.425,263.488,140.744,263.488z M210.913,131.744h-61.168V70.576c0-4.971-4.029-9-9-9s-9,4.029-9,9v61.168H70.576c-4.971,0-9,4.029-9,9s4.029,9,9,9h61.168 v61.168c0,4.971,4.029,9,9,9s9-4.029,9-9v-61.168h61.168c4.971,0,9-4.029,9-9S215.883,131.744,210.913,131.744z");
        newTicketImg.setOnMousePressed(e -> {
            if (e.isPrimaryButtonDown()) {
                clientViewModel.addNewTicket();
            }
        });
        newTicketLabel = imgLabel("New Ticket");

        GridPane newTicket = new GridPane();
        newTicket.addColumn(2, newTicketImg, newTicketLabel);
        newTicket.setPadding(new Insets(5));

        return new VBox(client, newTicket);
    ...
    public void setTicketListVisibility(boolean bool){
        userImg.setVisible(bool);
        userLabel.setVisible(bool);
    }

    public void setNewTicketVisibility(boolean bool){
        newTicketImg.setVisible(bool);
        newTicketLabel.setVisible(bool);
    }
